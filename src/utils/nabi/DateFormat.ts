...
export class DateFormat {
  ...
  public format(format: string): string {
    ...
    rv = rv.replace(/MM/g, Nabi.zerofill(this.date.getMonth() + 1, 2));
    rv = rv.replace(/dd/g, Nabi.zerofill(this.date.getDate(), 2));
    rv = rv.replace(/HH/g, Nabi.zerofill(this.date.getHours(), 2));
    rv = rv.replace(/mm/g, Nabi.zerofill(this.date.getMinutes(), 2));
    rv = rv.replace(/ss/g, Nabi.zerofill(this.date.getSeconds(), 2));
    rv = rv.replace(/zzz/g, Nabi.zerofill(this.date.getMilliseconds(), 3));
    return rv;
  }
