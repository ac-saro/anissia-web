...
export default class AslService {
  public static list(page: number, callback: (list: PageData<AslModel>) => void) {
    fetch(`/api/asl/any/list/${page}`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: {'Content-Type': 'application/json'},
    })
    .then((e) => e.json())
    .then((data) => callback(PageData.cast(data, AslModel.cast)));
  }
