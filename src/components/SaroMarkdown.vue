<template>
  <div :class="'saro-md-wrap' + (fullScreen ? ' md-screen-full' : '')">
    ...
    <div :class="`saro-md saro-md-body ${editMode}-view`" :style="style">
      <textarea ref="textarea" class="md-edit" :value="value" :placeholder="placeholder"
        @input="input($event)" 
        @keydown.tab="onTab" 
        @keydown.enter="onEnter"></textarea><div 
      class="md-edit md-view content" v-html="render(this.value)"></div>
    </div>
  </div>
</template>
...
export default Vue.extend({
  ...
  methods: {
    md()  {
      let md = (window as any)['markdown-it-core-md'];
      if (!md) {
    ...
    render(text: string): string {
      return (this.md().render(text) as string).split(/\&lt;br[ ]{0,1}\/\&gt;/g).join('<br/>');
    },
    input(event: Event) {
    ...
    onTab(event: KeyboardEvent) {
      ...
      if (sel.select.indexOf('\n') === -1) {
        ...
        // pre: `- ` | `* ` | `+ `
        if ((/^\s*[\+\-\*]{1} /).test(slLine) || (/^\s*1\. /).test(slLine)) {
          if (event.shiftKey) {
        ...
        const sl = this.getSelectLine(ta);
        if (event.shiftKey) {
          sl.select = sl.select.split('\n').map((e) => e.replace(/\t/g, '    ').replace(/^[ ]{1,4}/, '')).join('\n');
        } else {
          sl.select = sl.select.split('\n').map((e) => '    ' + e).join('\n');
        }
        this.setSelect(ta, sl);
